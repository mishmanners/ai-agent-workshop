...
If you already cloned it, open a terminal in your existing multi-channel-ai-agent-workshop` folder.
...
Use `python3` to create the virtual environment. After activation, use `python` and `python -m pip`; both commands will point to the Python 3 interpreter inside `.venv`. Note if you are using a Windows machine, you might need to use the following command.
...
Follow the steps below to create a local `.env` file and add your OpenAI API key.
